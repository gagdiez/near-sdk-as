...
@init
export function init(owner_id: string, metadata: NFTContractMetadata): void {
  assert(metadata.spec == "nft-1.0.0", "Invalid metadata spec");
...
@init
export function new_default_meta(owner_id: string): void {
...
@call({ payable: true })
export function nft_mint(token_id: string, token_owner_id: string, token_metadata: TokenMetadata, perpetual_royalties: JSON.Obj | null = null): void {
  mint.mint(token_id, token_owner_id, token_metadata);
}

@call({ payable: true })
export function nft_transfer(receiver_id: string, token_id: string, approval_id: string = "", memo: string = ""): void {
  assertOneYocto();
  nftCore.transfer(receiver_id, token_id, approval_id, memo);
}

@call({ payable: true })
export function nft_transfer_call(receiver_id: string, token_id: string, approval_id: string = "", memo: string = "", msg: string = ""): Promise {
  assertOneYocto();
  return nftCore.transferCall(receiver_id, token_id, approval_id, memo, msg);
}
...
@call({ payable: true })
export function nft_approve(token_id: string, account_id: string, msg: string = ""): u64 {
  return approval.approve(token_id, account_id);
}

@call({ payable: true })
export function nft_revoke(token_id: string, account_id: string): void { approval.revoke(token_id, account_id); }

@call({ payable: true })
export function nft_revoke_all(token_id: string): void { approval.revokeAll(token_id); }

@view
export function nft_is_approved(token_id: string, approved_account_id: string, approval_id: string = ""): bool {
  return approval.isApproved(token_id, approved_account_id, approval_id);
}
...
@view
export function nft_supply_for_owner(account_id: string): string { return enumeration.supplyForOwner(account_id).toString(); }

@view
export function nft_tokens_for_owner(account_id: string, from_index: u32 = 0, limit: u32 = 50): JsonToken[] {
  return enumeration.tokensForOwner(account_id, from_index, limit);
}
